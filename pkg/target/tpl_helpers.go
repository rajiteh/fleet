...
func (cc *tplTypeConversionContext) wrapped(valueType tplValueType, input any) tplTypedToken {
	str, ok := ensureConvertToStringsDeep(input, 0).(string)
...
func ensureConvertToStringsDeep(context any, recursionDepth int) any {
	val, err := convertToStringsDeep(context, recursionDepth)
	if err != nil {
		panic(err.Error())
	}
	return val
...
func convertToStringsDeep(context any, recursionDepth int) (any, error) {
	var err error
	if recursionDepth > maxTemplateRecursionDepth {
		return nil, fmt.Errorf("maximum recursion depth of %v exceeded. too many nested values", maxTemplateRecursionDepth)
	}
	...
	case map[string]any:
		...
		for key, val := range val {
			newMap[key], err = convertToStringsDeep(val, recursionDepth+1)
			if err != nil {
				return nil, err
			}
		}
	...
	case []any:
		...
		for i, v := range val {
			newSlice[i], err = convertToStringsDeep(v, recursionDepth+1)
			if err != nil {
				return nil, err
			}
		}
